...
Dialogs,Lua,MyWindows,ActiveX, MSScriptControl_TLB,MMSystem;
...
begin
   result:=False;
  if not TScriptThread.Running then
     begin
     lua_error(L);
     result:=True;
  end;
...
begin
  ...
  scr.AddCode('dim obj');
  scr.AddCode('set obj = createobject("QMDispatch.QMFunction")');
  scr.AddCode(format('FoundPixel = obj.FindColor(%s)',[_string]));
  scr.AddCode('x = CInt(FoundPixel / 8192)');
  scr.AddCode('y = FoundPixel Mod 8192');
...
var
  ...
  c1,c2:TColor;
begin
  c1:=Trunc(lua_tonumber(L,1));
  c2:=Trunc(lua_tonumber(L,2));
  lua_pop(L,2);
